jsPlumb.ready(function () {
    ...
    var blueBox = document.getElementById('.enableDisableSource')
    jsPlumb.on(blueBox, 'click', function (e) {
        var sourceDiv = (e.target || e.srcElement).parentNode
        var state = instance.toggleSourceEnable(sourceDiv)
    ...
    var greenBox = document.getElementById('canvas')
    jsPlumb.on(greenBox, 'click', function (e) {
        var targetDiv = (e.target || e.srcElement).parentNode
        var state = instance.toggleTargetEnable(targetDiv)
        this.innerHTML = (state ? 'disable' : 'enable')
        jsPlumb[state ? 'removeClass' : 'addClass'](targetDiv, 'element-disabled')
        jsPlumbUtil.consume(e)
    })

    // bind to connection event, just for the purposes of pointing out that it can be done 
    ...
    instance.batch(function () {
        ...
        instance.makeSource('sourceWindow1', {
            ...
            filterExclude: true,
            maxConnections: -1,
            endpoint: ['Dot', {radius: 7, cssClass: 'small-bluea'}],
        ...
        // and finally connect a couple of smass window , just so its obvious what's going on when this demo loads
        instance.connect({
